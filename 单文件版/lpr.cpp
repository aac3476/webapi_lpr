...
char * sendPic(char* addr, char * host, char *pathpic, char* picname, int port, string username, string psw) {
...
//※※※※※※※※※※※※※※※※※※※※※※※※※※※※※将main中代码完整复制到需要车牌识别的地方，将上方代码复制到需要车牌识别的文件顶部，其他部分只需要修改第240行即可！
...
int main() {
	...
	char * result = sendPic("/identify","152.32.134.49","D:\\1.jpg","1.jpg", 8801,"","");
	//↑只需要修改这个地方！第1 2个参数不用动，第三个参数为要识别的图片路径，第四个参数为要识别的图片名，第五个参数不要动，第六个参数为api_user，第七个参数为api_key，为保护接口安全，最后两个参数需要联系我提供
	cout << result;
	WSACleanup();
}